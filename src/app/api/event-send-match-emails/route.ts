  ...
  const matchRows = matches
    .map((m) => {
      const linkedinLink = m.linkedin
        ? `<a href="${m.linkedin}" style="color: #0a0a0a; text-decoration: underline;">LinkedIn Profile</a>`
        : "";
      const interests = m.profile.can_offer?.join(", ") || "";
      return `
        <tr>
          <td style="padding: 16px 0; border-bottom: 1px solid rgba(29,61,15,0.08);">
            <div style="font-size: 15px; font-weight: 600; color: #0a0a0a; margin-bottom: 2px;">
              ${m.rank}. ${m.profile.name}
            </div>
            <div style="font-size: 13px; color: #0a0a0a99; margin-bottom: 4px;">
              ${m.profile.role} at ${m.profile.company}
            </div>
            ${interests ? `<div style="font-size: 13px; color: #0a0a0a; margin-bottom: 4px;">
              <span style="background: #facc15; padding: 2px 8px; border-radius: 6px; font-weight: 500;">Can offer: ${interests}</span>
            </div>` : ""}
...
<body style="margin: 0; padding: 0; background-color: #ffffff; font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif;">
  <div style="max-width: 520px; margin: 0 auto; padding: 40px 24px;">
    <div style="text-align: center; margin-bottom: 32px;">
      <h1 style="color: #0a0a0a; font-size: 22px; font-weight: 700; margin: 0;">Your MatchUp Results</h1>
      <p style="color: #0a0a0a99; font-size: 14px; margin: 8px 0 0;">${eventName}</p>
    </div>
    <div style="background: #ffffff; border-radius: 16px; padding: 24px; border: 1px solid rgba(29,61,15,0.1);">
      <p style="color: #0a0a0a; font-size: 15px; margin: 0 0 4px;">Hi ${recipientName} 👋</p>
      <p style="color: #0a0a0a; font-size: 14px; margin: 0 0 20px; line-height: 1.5;">
    ...
    <div style="background: #ffffff; border-radius: 16px; padding: 24px; border: 1px solid rgba(29,61,15,0.1); margin-top: 20px;">
      <p style="color: #0a0a0a; font-size: 14px; font-weight: 600; margin: 0 0 4px;">While you wait to connect...</p>
      <p style="color: #0a0a0a99; font-size: 13px; margin: 0 0 16px;">Hear from founders who've been in your shoes.</p>
    ...
    <p style="text-align: center; color: #0a0a0a40; font-size: 12px; margin-top: 24px;">
      &copy; eventOS
    </p>
  </div>
</body>
